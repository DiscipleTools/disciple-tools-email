...
/**
 * Class Disciple_Tools_Email_Base
 * Load the core post type hooks into the Disciple.Tools system
 */
class Disciple_Tools_Email_Base extends DT_Module_Base {
    ...
    public function after_setup_theme(){
        $this->single_name = __( 'Starter', 'disciple-tools-email' );
        $this->plural_name = __( 'Starters', 'disciple-tools-email' );
    ...
    public function dt_get_post_type_settings( $settings, $post_type ){
        if ( $post_type === $this->post_type ){
            $settings['label_singular'] = __( 'Starter', 'disciple-tools-email' );
            $settings['label_plural'] = __( 'Starters', 'disciple-tools-email' );
        }
        return $settings;
    }
    ...
    public function dt_set_roles_and_permissions( $expected_roles ){
        ...
        if ( !isset( $expected_roles['multiplier'] ) ){
            $expected_roles['multiplier'] = [
                ...
                'label' => __( 'Multiplier', 'disciple-tools-email' ),
    ...
    public function dt_custom_fields_settings( $fields, $post_type ){
        if ( $post_type === $this->post_type ){
             ...
             */
            $fields['status'] = [
                'name'        => __( 'Status', 'disciple-tools-email' ),
                'description' => __( 'Set the current status.', 'disciple-tools-email' ),
                'type'        => 'key_select',
                'default'     => [
                    'inactive' => [
                        'label' => __( 'Inactive', 'disciple-tools-email' ),
                        'description' => __( 'No longer active.', 'disciple-tools-email' ),
                        'color' => '#F43636'
                    ],
                    'active'   => [
                        'label' => __( 'Active', 'disciple-tools-email' ),
                        'description' => __( 'Is active.', 'disciple-tools-email' ),
                        'color' => '#4CAF50'
                    ],
                ],
                'tile'     => 'status',
            ...
            $fields['assigned_to'] = [
                'name'        => __( 'Assigned To', 'disciple-tools-email' ),
                'description' => __( 'Select the main person who is responsible for reporting on this record.', 'disciple-tools-email' ),
                'type'        => 'user_select',
                'default'     => '',
                'tile' => 'status',
             ...
             */
            $fields['start_date'] = [
                'name'        => __( 'Start Date', 'disciple-tools-email' ),
                'description' => '',
                'type'        => 'date',
                'default'     => time(),
                'tile' => 'details',
            ...
            $fields['end_date'] = [
                'name'        => __( 'End Date', 'disciple-tools-email' ),
                'description' => '',
                'type'        => 'date',
                'default'     => '',
                'tile' => 'details',
            ...
            $fields['multi_select'] = [
                'name' => __( 'Multi-Select', 'disciple-tools-email' ),
                'description' => __( 'Multi Select Field', 'disciple-tools-email' ),
                'type' => 'multi_select',
                'default' => [
                    'item_1' => [
                        'label' => __( 'Item 1', 'disciple-tools-email' ),
                        'description' => __( 'Item 1.', 'disciple-tools-email' ),
                    ],
                    'item_2' => [
                        'label' => __( 'Item 2', 'disciple-tools-email' ),
                        'description' => __( 'Item 2.', 'disciple-tools-email' ),
                    ],
                    'item_3' => [
                        'label' => __( 'Item 3', 'disciple-tools-email' ),
                        'description' => __( 'Item 3.', 'disciple-tools-email' ),
                    ],
                ],
                'tile' => 'details',
                'in_create_form' => true,
            ...
            $fields['location_grid'] = [
                'name'        => __( 'Locations', 'disciple-tools-email' ),
                'description' => __( 'The general location where this contact is located.', 'disciple-tools-email' ),
                'type'        => 'location',
                'mapbox'    => false,
                'in_create_form' => true,
                'tile' => 'details',
            ...
            $fields['location_grid_meta'] = [
                'name'        => __( 'Locations', 'disciple-tools-email' ), //system string does not need translation
                'description' => __( 'The general location where this record is located.', 'disciple-tools-email' ),
                'type'        => 'location_meta',
                'tile'      => 'details',
                'mapbox'    => false,
                'hidden' => true,
            ...
            $fields['contact_address'] = [
                'name' => __( 'Address', 'disciple-tools-email' ),
             ...
             */
            $fields['parents'] = [
                'name' => __( 'Parents', 'disciple-tools-email' ),
                'description' => '',
                'type' => 'connection',
            ...
            $fields['peers'] = [
                'name' => __( 'Peers', 'disciple-tools-email' ),
                'description' => '',
                'type' => 'connection',
                'post_type' => $this->post_type,
                'p2p_direction' => 'any',
            ...
            $fields['children'] = [
                'name' => __( 'Children', 'disciple-tools-email' ),
                'description' => '',
                'type' => 'connection',
                'post_type' => $this->post_type,
                'p2p_direction' => 'to',
             ...
             */
            $fields['peoplegroups'] = [
                'name' => __( 'People Groups', 'disciple-tools-email' ),
                'description' => __( 'The people groups connected to this record.', 'disciple-tools-email' ),
                'type' => 'connection',
                'tile' => 'details',
            ...
            $fields['contacts'] = [
                'name' => __( 'Contacts', 'disciple-tools-email' ),
                'description' => '',
                'type' => 'connection',
                'post_type' => 'contacts',
                'p2p_direction' => 'to',
    ...
    public function dt_details_additional_tiles( $tiles, $post_type = '' ){
        if ( $post_type === $this->post_type ){
            $tiles['connections'] = [ 'label' => __( 'Connections', 'disciple-tools-email' ) ];
            $tiles['other'] = [ 'label' => __( 'Other', 'disciple-tools-email' ) ];
        }
        return $tiles;
    }
    ...
    public function dt_details_additional_section( $section, $post_type ){
        ...
        if ( $post_type === $this->post_type && $section === 'other' ) {
            ...
            <div class="section-subheader">
                <?php esc_html_e( 'Custom Section Contact', 'disciple-tools-email' ) ?>
            </div>
            <div>
...
//        }
    ...
    public static function dt_user_list_filters( $filters, $post_type ){
        ...
        if ( $post_type === self::post_type() ){
            ...
            $filters['tabs'][] = [
                'key' => 'assigned_to_me',
                'label' => __( 'Assigned to me', 'disciple-tools-email' ),
                'count' => $total_my,
                'order' => 20
            ];
            ...
            $filters['filters'][] = [
                'ID' => 'my_all',
                'tab' => 'assigned_to_me',
                'name' => __( 'All', 'disciple-tools-email' ),
                'query' => [
                    'assigned_to' => [ 'me' ],
                    'sort' => 'status'
                ],
            ...
            if ( current_user_can( 'view_any_' . self::post_type() ) ){
                ...
                $filters['tabs'][] = [
                    'key' => 'all',
                    'label' => __( 'All', 'disciple-tools-email' ),
                    'count' => $total_all,
                    'order' => 10
                ];
                // add assigned to me filters
