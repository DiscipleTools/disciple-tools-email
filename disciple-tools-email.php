<?php
/**
 * Plugin Name: Disciple.Tools - D.T Email
 * Plugin URI: https://github.com/DiscipleTools/disciple-tools-email
 * Description: Disciple.Tools - D.T Email is intended to help developers and integrator jumpstart their extension of the Disciple.Tools system.
 * Text Domain: disciple-tools-email
 * Domain Path: /languages
 * Version:  0.1
 * Author URI: https://github.com/DiscipleTools
 * GitHub Plugin URI: https://github.com/DiscipleTools/disciple-tools-email
 ...
 * 1. @todo Rename the `disciple-tools-email.php file.
 * 2. @todo Refactor all occurrences of the name Disciple_Tools_Email, disciple_tools_email, disciple-tools-email, dt_email, and "D.T Email"
...
/**
 * Gets the instance of the `Disciple_Tools_Email` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_email() {
    $disciple_tools_email_required_dt_theme_version = '1.19';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_email_required_dt_theme_version, '<' ) ) {
        add_action( 'admin_notices', 'disciple_tools_email_hook_admin_notice' );
    ...
    return Disciple_Tools_Email::instance();

}
add_action( 'after_setup_theme', 'disciple_tools_email', 20 );

//register the D.T Plugin
add_filter( 'dt_plugins', function ( $plugins ){
    $plugin_data = get_file_data( __FILE__, [ 'Version' => 'Version', 'Plugin Name' => 'Plugin Name' ], false );
    $plugins['disciple-tools-email'] = [
...
class Disciple_Tools_Email {
    ...
    private function __construct() {
         ...
         */
        if ( $is_rest && strpos( dt_get_url_path(), 'dt-email' ) !== false ) {
         ...
         */
        if ( strpos( dt_get_url_path(), 'metrics' ) !== false || ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-email-metrics' ) !== false ) ){
...
//        require_once( 'magic-link/magic-link-home.php' );
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-email' );
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( 'disciple_tools_email::' . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
register_activation_hook( __FILE__, [ 'Disciple_Tools_Email', 'activation' ] );
register_deactivation_hook( __FILE__, [ 'Disciple_Tools_Email', 'deactivation' ] );


if ( ! function_exists( 'disciple_tools_email_hook_admin_notice' ) ) {
    function disciple_tools_email_hook_admin_notice() {
        global $disciple_tools_email_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        $current_version = $wp_theme->version;
        $message = "'Disciple.Tools - D.T Email' plugin requires 'Disciple.Tools' theme to work. Please activate 'Disciple.Tools' theme or make sure it is latest version.";
        if ( $wp_theme->get_template() === 'disciple-tools-theme' ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple.Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $disciple_tools_email_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-email', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-email is-dismissible" data-notice="disciple-tools-email">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-email .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-email',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-email/wiki/Configuring-Remote-Updating-System
 *
...
//                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-email/master/version-control.json',
//                __FILE__,
//                'disciple-tools-email'
//            );
//
//        }
//    }
